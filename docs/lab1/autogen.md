...
│   ├── ...
|   ├── cminusfc
|   |    └── cminusf_builder.hpp        <- 该阶段(可能)需要修改的文件
│   ├── lightir/*
│   └── common
│        ├── ast.hpp
│        ├── logging.hpp
...
实验框架实现了 Lab1 生成的分析树到 C++ 上的抽象语法树的转换。可以使用[访问者模式](./visitor_pattern.md)来实现对抽象语法树的遍历，[ast.hpp](https://github.com/USTC-Compiler-2025/2025ustc-compiler/blob/main/include/common/ast.hpp) 文件中包含抽象语法树节点定义。
...
`CminusfBuilder` 类定义在 [cminusf_builder.hpp](https://github.com/USTC-Compiler-2025/2025ustc-compiler/tree/main/include/cminusfc/cminusf_builder.hpp)文件中，`CminusfBuilder` 类中定义了对抽象语法树不同语法节点的 `visit` 函数，实验已给出了一些语法树节点的访问规则，其余的需要学生补充。
